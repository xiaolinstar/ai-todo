...
> MCP 不应比 CLI 更难装。npm 包 `@xiaolinstar/ai-todo-mcp` 发布前，需仓库内 `pnpm mcp:build`；发布后默认用 `npx`（见「安装 MCP」）。
...
# 发布后：npx -y @xiaolinstar/ai-todo-mcp  （v0.6.2.1 计划）
```

宿主配置示例（**npm 发布后**，各宿主通用）：
