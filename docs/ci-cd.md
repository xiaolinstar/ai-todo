...
| **CD** | `.github/workflows/cd.yml` | 仅手动 `workflow_dispatch` | 校验指定 CI 制品指纹 → 拉取 CI 镜像部署（不在 VPS 上 build） |
...
本地 main 或短特性分支开发
  ...
  → 微信小程序人工上传体验版/提审
  → 确认客户端与服务端版本节奏一致后
  → 手动触发 CD，填写目标 CI run id：按 manifest 部署生产
```
...
- main 上 CI 是否生成了 `deploy-manifest`，并记录本次 CI run id。
- 是否已经上传/验收匹配的小程序体验版。
- 手动 CD 是否完成部署。
...
`cd.yml` 需 `permissions.actions: read`，以便手动 CD 根据 `ci_run_id` 下载 CI 上传的 `deploy-manifest` 制品。
...
- `ci_run_id`：指定某次成功 CI 的 run id。该 CI 必须产出 `deploy-manifest`。
