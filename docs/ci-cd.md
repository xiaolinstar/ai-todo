...
| Job | 说明 |
|-----|------|
| `resolve-manifest` | 解析 tag / `ci_run_id`，校验 manifest 指纹 |
| `deploy` | SSH 部署；VPS 本机 `/v1/health` + `/v1/health/db`（L0）；成功时写 `.deploy/previous-success.json` |
| `post-deploy-verify` | 对 `CD_PUBLIC_API_URL` 公网拨测；可选 `CD_SMOKE_PAT` 测 `/v1/me`、`/v1/today` |
| `rollback` | 仅当 deploy 成功且 post-deploy-verify 失败；SSH `AI_TODO_CD_ACTION=rollback` |
| `post-rollback-verify` | 回滚后公网 health/db（不校验 gitSha） |
| `release-report` | `always()` 生成 `cd-release-report` artifact 与 Step Summary |

实现细节：[releases/v0.2.3-cd-pipeline-plan.md](./releases/v0.2.3-cd-pipeline-plan.md)。
...
| GitHub Environments | 仓库内 **仅配置了 `production`**（2026-06-04）；**无 `staging` Environment** |
| staging 是否在跑 | **否**。选 staging 时不会单独连另一台机，只会用**仓库级** Secrets（若未建 Environment `staging`），极易与 production **同一 VPS** |
| 启用 staging 前 | 在 GitHub → Settings → Environments 新建 `staging`，配置独立 `DEPLOY_*`、`CD_PUBLIC_API_URL`，再手动 CD 选 staging 验证 |

`resolve-manifest` 在选 staging 时会打出 warning，提醒先创建 Environment。

### Secret 清单（按存放位置）

**staging 当前不启用**：手动 CD 时 `environment` 默认为 **`production`**（workflow 已配置），保持默认即可，勿改选 staging。
