...
| **CD**                 | `.github/workflows/cd.yml`               | 仅手动 `workflow_dispatch` | Compose 部署：校验 manifest → 部署 → 验收 →（失败）回滚            |
| **CD (K8s)**           | `.github/workflows/cd-k8s.yml`           | 仅手动 `workflow_dispatch` | K8s 应用部署：只发布 API / worker，不管理 Postgres，不跑 migration |
| **DB Migration (K8s)** | `.github/workflows/db-migration-k8s.yml` | 仅手动 `workflow_dispatch` | K8s 数据库迁移：复用 manifest 镜像，显式运行 Alembic Job           |
...
Git tag 在 CD 中只作为 **`release_tag` 输入参数 / 版本锚点**：workflow 会解析 tag → commit → 该 commit 的成功 CI → `deploy-manifest`。不要把 tag push 作为 CD 触发器，否则 tag 创建可能早于 CI `publish-manifest` 完成，导致 CD 找不到可部署 manifest；也可能绕过 staging / 小程序体验版 / 人工晋升节奏。
...
- 涉及数据库 migration 时，必须遵守 [database-migrations.md](./database-migrations.md)。K8s 环境下 migration 通过 `DB Migration (K8s)` 显式执行，不随 `CD (K8s)` 自动运行。
...
日常发布（**推荐**）：先 Run `CD` → `environment=staging` → 填 **`release_tag`** → 验收与浸泡后，再 Run `CD (K8s)` → GitHub Environment `production-k8s`（同一 tag）。晋升门槛见 [staging-production-promotion.md](./releases/staging-production-promotion.md)。

`ci_run_id` 仅用于回滚或救急，见 [v0.2.3-cd-workflow-redesign.md](./releases/v0.2.3-cd-workflow-redesign.md)。

禁止未经设计评审为 CD / CD (K8s) 增加 `push.tags`、`workflow_run` 等自动部署触发。若未来需要自动化发布，必须先重新设计与小程序审核、staging 晋升、DB migration 和 manifest race 的关系。

### CD Job 流程（分支式 DAG）

```text
1 Resolve → 2 Classify → 3 Deploy → 4 Verify
                            ├─ 5a Outcome published     （Verify 通过）
                            ├─ 5c Outcome deploy failed （Deploy 失败）
                            └─ 5b Rollback → 6b Verify after rollback → 7b Outcome not adopted
8 Release report（always，汇合）
```

| 显示名                    | 说明                                                                                |
| ------------------------- | ----------------------------------------------------------------------------------- |
| `1 · Resolve manifest`    | tag / `ci_run_id` → manifest                                                        |
| `2 · Classify release`    | 比较当前线上 `gitSha` 与本次 manifest，识别 app-only / schema-change / infra-change |
| `3 · Deploy`              | SSH；VPS L0 health；写 `previous-success.json`                                      |
| `4 · Verify (public API)` | `CD_PUBLIC_API_URL` 拨测 / 黑盒                                                     |
| `5a · Outcome published`  | 成功分支终点                                                                        |
| `5b–7b`                   | 失败时回滚 + 复验 +「未采纳」                                                       |
| `8 · Release report`      | artifact + Summary                                                                  |
...
| 项                  | 现状                                                                                                           |
| ------------------- | -------------------------------------------------------------------------------------------------------------- |
| workflow 输入       | Compose CD 可选 `environment: staging`；K8s CD 固定 GitHub Environment `production-k8s`                        |
| GitHub Environments | 已配置 **`staging`** 与 **`production-k8s`**，各自独立 `DEPLOY_*`、`CD_PUBLIC_API_URL` / `CD_LOCAL_HEALTH_URL` |
| 小程序映射          | 体验版 / 预览 / 真机调试 → staging API；正式版 → production API                                                |
| 晋升策略            | **staging Compose 先发布、达标后再 production K8s**（当前人工 checklist；自动化门禁待实现）                    |
...
**推荐**：`staging` 与 `production-k8s` 各自在 GitHub Environment 中配置 `DEPLOY_*`、`CD_PUBLIC_API_URL` / `CD_LOCAL_HEALTH_URL`；勿让两环境共用同一 VPS 除非刻意为之。

#### A. GitHub Secrets（Actions 用，不在 VPS 上配置）

旧 Compose CD 里带 `environment: production` 的 Job **同时可读**。当前生产发布入口已迁到 `CD (K8s)` / `production-k8s`，该旧环境只作为历史兼容和回退参考：
...
CD 在部署前运行 `classify-release`：读取目标环境公网 `/v1/health` 的当前 `gitSha`，与本次 manifest `gitSha` 做 diff。若发现 Alembic migration 变更，Actions Summary 与 release report 会提示生产备份，以及“应用回滚不会自动回滚 PostgreSQL schema/data”的边界。

## CD 与多环境

| 环境           | GitHub Environment | 触发方式                                      | Secrets                                    |
| -------------- | ------------------ | --------------------------------------------- | ------------------------------------------ |
| **staging**    | `staging`          | `CD` workflow，选择 staging                   | 在 Environment 中配置独立 `DEPLOY_*`       |
| **production** | `production-k8s`   | `CD (K8s)` workflow                           | `DEPLOY_*`、`K8S_*`、`CD_LOCAL_HEALTH_URL` |
| production old | `production`       | Compose 旧生产 / 回退参考，不作为当前发布入口 | 历史 `DEPLOY_*`、`GHCR_DEPLOY_*`           |
...
启用 Compose `CD` 前，在 GitHub **Settings → Secrets and variables → Actions**（仓库级即可；staging 推荐放在 Environment `staging`）配置：
...
- `environment`：当前日常只使用 `staging`；`production` 是旧 Compose 生产/回退入口，不作为当前生产发布路径。
- `ci_run_id`：可选。仅在回滚或明确指定某次 CI 时填写；该 CI 必须产出 `deploy-manifest`。
- `release_tag`：日常发布必填，例如 `v0.1.1`。这是运行参数，不是触发事件；CD 会解析 tag 指向的 commit，并查找该 commit 对应的成功 CI。
...
3. `environment=staging`，`release_tag=v0.1.1`，`ci_run_id` 留空，`deploy_mode=auto`。
4. CD 会解析 `v0.1.1` 指向的 commit，自动查找该 commit 对应的最新成功 `CI` push run，下载 `deploy-manifest` 并部署到 staging Compose。

Production 日常发布使用 `CD (K8s)` workflow，填写同一 `release_tag` 或 `ci_run_id`，由 `production-k8s` Environment 提供部署变量。
...
Compose manifest 部署会在替换 API 后**轮询** `/v1/health` 与 `/v1/health/db`。K8s `CD (K8s)` 只发布 API / worker，并验证健康检查；Alembic migration 已拆到 `DB Migration (K8s)`。若新版本启动或健康检查失败，脚本会读取部署前的 `.deploy/current.json`，自动执行应用层回滚：
...
这个回滚恢复的是**应用代码 + 容器镜像**，不会回滚 PostgreSQL volume / PVC 内已经执行过的数据库迁移。因此数据库变更必须遵守向前兼容迁移：先 expand，再 deploy/backfill，最后 contract。破坏性 schema 变更不能和应用发布绑在一次不可回滚的部署里。
...
## Node.js 24（Actions 运行时，零弃用告警）

**要求：** CI/CD/Monitor 工作流完成时，Annotations 中不得出现 `Node.js 20 is deprecated` 类告警。仅设置 `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` **不够**——仍会告警；须使用声明 `using: node24` 的 action 版本。
...
当前 pin（均 node24 runtime，2026-06 复核）：

| Action                       | 版本                  |
| ---------------------------- | --------------------- |
| `actions/checkout`           | `@v6`                 |
| `gitleaks/gitleaks-action`   | `@v3`（v2 仍 node20） |
| `actions/setup-node`         | `@v5`                 |
| `actions/setup-python`       | `@v6`                 |
| `actions/upload-artifact`    | `@v6`                 |
| `actions/download-artifact`  | `@v7`                 |
| `pnpm/action-setup`          | `@v5`                 |
| `docker/setup-buildx-action` | `@v4`（v3 仍 node20） |
| `docker/login-action`        | `@v4`                 |
| `docker/build-push-action`   | `@v7`                 |

排查：若仍出现 node20 告警，在 job 摘要搜索 `The following actions`，逐项升到 node24 版本；`appleboy/*` 为 composite（shell），通常不触发此类告警。
