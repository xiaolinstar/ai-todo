...
| 无对外 URL 拨测 | `https://wodi.games`（或日后 `xingxiaolin.cn`）与 localhost 不一致时抓不到 |
...
| Secret / 变量 | 用途 |
|---------------|------|
| `CD_PUBLIC_API_URL` | 对外基址，如 `https://wodi.games`（Environment 级，staging/production 各配） |
...
- 拨测 URL 使用 Environment 中的 `CD_PUBLIC_API_URL`，**不硬编码** `wodi.games` / `xingxiaolin.cn`
- 网关双域并行时，production 可先继续填 `https://wodi.games`，切换域名只改 Secret，无需改 workflow 结构
