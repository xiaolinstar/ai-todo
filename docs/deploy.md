...
详见 **[docs/ci-cd.md](./ci-cd.md)**（阶段划分、制品指纹、多环境、GHCR）。

| 工作流 | 文件 | 触发 |
|--------|------|------|
| CI | `.github/workflows/ci.yml` | push / PR → `main` |
| CD | `.github/workflows/cd.yml` | `main` CI 成功 + `deploy-manifest` 制品；或手动指定环境 |

**CD Secrets（production / staging Environment）：**

| Secret | 说明 |
|--------|------|
| `DEPLOY_HOST` / `DEPLOY_USER` / `DEPLOY_SSH_KEY` / `DEPLOY_PATH` | SSH 部署 |
| `GHCR_DEPLOY_TOKEN` | 拉取 `ghcr.io/xiaolinstar/ai-todo-api`（read:packages PAT） |
| `GHCR_DEPLOY_USER` | 可选，默认仓库 owner |
