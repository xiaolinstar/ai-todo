...
| **处理** | ① GHCR 包改 **Public**；② `.env` 或 CD 注入 `AI_TODO_PULL_REGISTRY_MIRROR=ghcr.nju.edu.cn`；③ 单次 pull `timeout` + 少次数重试；④ 镜像站失败后默认回退官方 `ghcr.io`；⑤ 官方也失败后 **server-build 兜底** |
...
### 2.8 tag push 自动触发 CD 导致失败 / 抢跑 CI manifest

| 项       | 说明                                                                                                                                     |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **现象** | push `vX.Y.Z` tag 后 `CD` / `CD (K8s)` 自动出现红色 run；日志可能有 `No successful CI push run found`，或前置 job skipped 后 report 失败 |
| **原因** | tag push 触发 CD 时，目标 commit 的 CI 可能还未完成 `publish-manifest`；自动部署也绕过 staging 验收、小程序体验版和人工晋升门槛          |
| **处理** | CD workflow 只保留 `workflow_dispatch`；日常发布手动 Run workflow 并填写 `release_tag=vX.Y.Z`，`ci_run_id` 留空                          |
| **预防** | 不给 CD / CD (K8s) 增加 `push.tags` 或 `workflow_run` 自动触发；若未来要自动化，先设计 manifest 等待、staging gate、小程序审核和 DB gate |
...
| Pull             | 2 次重试、单次 timeout 约 180s、镜像站失败后回退官方 GHCR |
| 健康检查等待     | 90–120s                                                   |
...
| `No successful CI push run found`                  | CD 被 tag push 抢跑；等待 CI 绿后手动填 tag    |
