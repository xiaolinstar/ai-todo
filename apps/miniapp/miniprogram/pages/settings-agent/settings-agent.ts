...
import { isActiveTokenStatus, normalizeApiTokenSummary } from '../../lib/token-status';
...
Page({
  ...
  onCreateToken() {
    ...
    wx.showActionSheet({
      itemList: [...TOKEN_PRESETS.map((item) => item.actionLabel), '自定义名称和选项'],
      success: (result) => {
  ...
  onOpenHelp() {
    wx.showModal({
      title: 'CLI 配置',
      content:
        '点「新建令牌」选择用途即可快速创建。创建后复制配置到电脑 ~/.ai-todo/settings.json。完整令牌只在创建成功时显示一次。',
      showCancel: false,
    });
  },
  ...
  onOpenDetail(e: { currentTarget: { dataset: { id: string } } }) {
    ...
    wx.navigateTo({
      url: `/pages/settings-token-detail/settings-token-detail?id=${encodeURIComponent(id)}`,
    });
  },
});
